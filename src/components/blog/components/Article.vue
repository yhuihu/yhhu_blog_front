<template>
  <div id="article"
       class="article">
    <h1 id="title"
        class="title"> {{ blog.title }}</h1>
    <div v-if="blog.typeName !== '原创' && blog.originalUrl"
         class="origin-wrapper">
      <a v-if="blog.typeName === '转载'"
         :href="blog.originalUrl"
         target="_blank"
         class="origin">转自{{ blog.originalUrl
        }}</a>
      <a v-if="blog.typeName === '翻译'"
         :href="blog.originalUrl"
         target="_blank"
         class="origin">译自{{ blog.originalUrl
        }}</a>
    </div>
    ...
    <div style="text-align: center; margin-top: 40px">
      <button id="like-btn"
              class="btn btn-like"
              @click="addLike">喜欢 {{ blog.likeCount }}</button>
    </div>
  </div>
</template>
